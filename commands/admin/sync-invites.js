...
class SyncInvites extends Command {
    ...
    async run (message, args, data) {
        ...
        let conf = await message.channel.send(message.language.syncinvites.confirmations.all(data.guild.prefix, invitesCount));
        await message.channel.awaitMessages((m) => m.author.id === message.author.id && (m.content === "cancel" || m.content === "-confirm"), { max: 1, time: 90000 }).then(async (collected) => {
            ...
            .setDescription(message.language.restoreinvites.titles.all(data.guild.prefix))
            .setColor(data.color)
