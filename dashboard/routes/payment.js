        ...
        if (payload.txn_type === "subscr_payment") {
            ...
            req.client.users.fetch(userID).then(async (user) => {
                ...
                if (signupData) {
                    ...
                    } else await req.client.database.updateGuildSubscription(currentSubscription.id, guildID, {
                        expiresAt: new Date((new Date(currentSubscription.expiresAt).getTime() > Date.now() ? new Date(currentSubscription.expiresAt).getTime() : Date.now()) + additionalTime).toISOString()
                    });
